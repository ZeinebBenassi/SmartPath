...
public class QuizResultController implements Initializable {
    ...
    // ═══════════════════════════════════════════════════════════════════
    //  Carte — Tuiles OSM chargées côté Java (bypass WebView network)
    // ═══════════════════════════════════════════════════════════════════

    /** Convertit lon → numéro de tuile OSM */
    private int lon2tile(double lon, int z) {
        return (int) Math.floor((lon + 180.0) / 360.0 * (1 << z));
    }

    /** Convertit lat → numéro de tuile OSM */
    ...
    /**
     * Télécharge une tuile OSM via Java (réseau JVM, pas WebView)
     * et la retourne encodée en base64 data-URL.
     */
    private String tuileEnBase64(int z, int x, int y) {
        ...
        try {
            ...
            conn.setRequestProperty("User-Agent",
                "Mozilla/5.0 SmartPathApp/1.0 (contact@smartpath.tn)");
     ...
     * Les tuiles sont téléchargées en Java puis injectées en base64 dans le HTML
     * → aucune requête réseau depuis le WebView JavaFX (problème contourné).
     */
    private void ouvrirCarte(String nom, String ville, String mapQuery,
        ...
        try {
            ...
            // ── Pré-charger une grille 5×5 de tuiles en Java ──
            ...
            // ── Construire le HTML carte canvas ──
            String html = buildMapHtml(lat, lon, zoom, found, nomJs, villeJs,
                                       tilesJson.toString());
            ...
            // WebView — chargement du HTML avec tuiles base64 embarquées
            ...
            btnF.setStyle("-fx-background-color:white;-fx-text-fill:#64748B;"
    ...
    /**
     * Génère le HTML de la carte canvas.
     * Les tuiles OSM sont injectées en base64 → aucune dépendance réseau externe.
     * Le WebView JavaFX peut charger des images depuis src="data:..." sans restriction.
     */
    private String buildMapHtml(double lat, double lon, int zoom, boolean found,
        ...
        return "<!DOCTYPE html><html><head>"
            ...
            + "    } else {\n"
            // Tuile non pré-chargée → fond neutre
