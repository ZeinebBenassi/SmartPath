...
public class CoursController {
    ...
    public void initialize() {
        ...
        if (matiereFilter != null) {
            matiereFilter.getItems().clear();
            matiereFilter.setPromptText("Filtrer");
        }

        if (leconsList != null) {
            leconsList.getItems().clear();
            leconsList.setPlaceholder(new Label("Aucune leçon disponible pour le moment."));
        }
    ...
    @FXML public void addLecon() {
        navigate("/tn/esprit/interfaces/LeconForm.fxml");
    }
    ...
    @FXML public void deleteLecon() {
        new Alert(Alert.AlertType.INFORMATION,
            "Aucune leçon à supprimer pour le moment.", ButtonType.OK).showAndWait();
    }

    @FXML public void telechargerFichier() {
        new Alert(Alert.AlertType.INFORMATION, "Aucun fichier à télécharger pour le moment.", ButtonType.OK).showAndWait();
    }

    @FXML public void goBack() {
        String dest = (currentUser != null && "prof".equals(currentUser.getType()))
            ? "/tn/esprit/interfaces/DashboardProf.fxml" : "/tn/esprit/interfaces/DashboardEtudiant.fxml";
        navigate(dest);
    }
