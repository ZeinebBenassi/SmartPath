...
public class QuizFormController implements NavigableController {
        ...
        if (existing == null) {
            ...
            if (matiereContextLabel != null) {
                if (createMatiereId != null && createMatiereId > 0) {
                    String matTitle = (createMatiereTitle == null || createMatiereTitle.isBlank()) ? "Matière sélectionnée" : createMatiereTitle;
            ...
            if (matiereContextLabel != null) {
                matiereContextLabel.setText("Matière associée au quiz");
            }
        }
    }
    ...
    public void handleSave() {
        if (!AccessControl.canManageContent(role)) {
            if (errorLabel != null) errorLabel.setText("Accès refusé.");
            return;
        }
        ...
        int matiereId = existing == null ? (createMatiereId == null ? 0 : createMatiereId) : existing.getMatiereId();

        if (titre.isBlank() || duree <= 0 || matiereId <= 0) {
            if (errorLabel != null) errorLabel.setText("Veuillez remplir tous les champs obligatoires.");
            return;
        }
        ...
        try {
            if (existing == null) {
                ...
                int quizId = service.createAndReturnId(q, AppSession.getCurrentUser().getId());
                if (quizId > 0) createQuestionsFlow(quizId);
            } else {
                existing.setTitre(titre); existing.setContenu(contenu); existing.setDuree(duree);
    ...
    private void createQuestionsFlow(int quizId) {
        ...
        for (int i = 1; i <= count; i++) {
            ...
            if (draftOpt.isEmpty()) break;

            QuestionDraft draft = draftOpt.get();
            try {
                Question q = new Question();
                q.setText(draft.text + " (Rép: " + draft.answer + ")");
                q.setCategory("Vrai/Faux");
                q.setOrdre(i); q.setActive(true); q.setTestId(quizId);
                questionService.createForTestId(quizId, q, AppSession.getCurrentUser().getId());
            } catch (SQLException e) {
                new Alert(Alert.AlertType.ERROR, e.getMessage()).showAndWait();
                break;
            }
    ...
    private int askQuestionCount() {
        var dialog = new TextInputDialog("3");
        dialog.setTitle("Nombre de questions");
        dialog.setHeaderText("Combien de questions ? (Max 5)");
        Optional<String> value = dialog.showAndWait();
        if (value.isEmpty()) return 0;
        try {
            int n = Integer.parseInt(value.get().trim());
            return Math.max(1, Math.min(n, 5)); // Limit to 5
    ...
    private Optional<QuestionDraft> askOneQuestion(int index, int total) {
        ...
        ButtonType save = new ButtonType("Suivant", ButtonBar.ButtonData.OK_DONE);
        dialog.getDialogPane().getButtonTypes().addAll(save, ButtonType.CANCEL);

        TextArea text = new TextArea(); text.setPromptText("Énoncez la question...");
        text.setPrefRowCount(3);

        ToggleGroup group = new ToggleGroup();
        RadioButton rbVrai = new RadioButton("Vrai"); rbVrai.setToggleGroup(group); rbVrai.setSelected(true);
        RadioButton rbFaux = new RadioButton("Faux"); rbFaux.setToggleGroup(group);

        VBox box = new VBox(10, new Label("Question :"), text, new Label("Réponse correcte :"), new HBox(20, rbVrai, rbFaux));
        dialog.getDialogPane().setContent(box);

        dialog.setResultConverter(btn -> {
            if (btn == save) {
                String ans = ((RadioButton)group.getSelectedToggle()).getText();
                return new QuestionDraft(safe(text.getText()), ans);
            }
            return null;
        });
    ...
    private record QuestionDraft(String text, String answer) {}
