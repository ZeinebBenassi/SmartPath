...
public class DashboardEtudiantController {
    ...
    public static void setSourceDashboardType(String r) { sourceDashboardType = r; }
    ...
    // ─── Quiz → ouvre le VRAI QuizPlayer avec données BDD ────────────────────
    @FXML public void showQuiz() {
        setActiveButton(btnQuiz);
        try {
            // Passer l'utilisateur connecté au quiz
            QuizPlayerController.setCurrentUser(currentUser);
            ...
            System.out.println("Erreur ouverture quiz : " + e.getMessage());
            e.printStackTrace();
        }
    }
    ...
    private void setActiveButton(Button active) {
        ...
        for (Button b : all) {
            if (b == null) continue;
            if (b == active) {
                b.setStyle("-fx-background-color: #eff6ff; -fx-text-fill: #2563eb; -fx-font-weight: bold;" +
                           "-fx-font-size: 13; -fx-padding: 10 16; -fx-background-radius: 8; -fx-cursor: hand;" +
                           "-fx-alignment: CENTER_LEFT; -fx-border-color: #bfdbfe; -fx-border-radius: 8;");
            } else {
                b.setStyle("-fx-background-color: transparent; -fx-text-fill: #5a6a8a; -fx-font-size: 13;" +
                           "-fx-padding: 10 16; -fx-background-radius: 8; -fx-cursor: hand; -fx-alignment: CENTER_LEFT;");
            }
        }
    }
}
