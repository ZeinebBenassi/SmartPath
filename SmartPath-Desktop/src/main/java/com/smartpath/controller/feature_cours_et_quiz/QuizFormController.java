...
import com.smartpath.service.QuestionService;
...
public class QuizFormController implements NavigableController {
    ...
    public void handleSave() {
        ...
        if (duree <= 0) {
            errorLabel.setText("La durée doit être un nombre > 0.");
            return;
        }
        if (matiereId <= 0) {
            errorLabel.setText("Créez le quiz depuis la page Matières (Ajouter Quiz). ");
            return;
        }

        if (contenu.length() > 2000) {
            errorLabel.setText("Contenu trop long (max 2000 caractères).");
            return;
        }
